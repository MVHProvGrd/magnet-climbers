...
| `22-avatars-v1/` | 86 cartoon sticker avatars for chat and the profile (fridge crew, snacks, creatures, whimsical people). `gen.py` (Gemini, magenta), cut with the chroma-cutout skill (`--key=auto`, pink subjects generated on green), `sheet.py` packs the sprite sheet. | Claude via Gemini 2.5 Flash Image | LIVE: `public/art/avatars/sheet.webp`, picker in Settings, shown in chat and the menu ticker |
